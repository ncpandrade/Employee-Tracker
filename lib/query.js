...
const queries = {
    viewEmployees: `SELECT e.id, e.first_name, e.last_name, r.title, departments.department_name as department, r.salary, IFNULL(emp.last_name,'top manager') as manager 
                    FROM employees e
                    LEFT JOIN roles r     
                    ON r.id = e.role_id
                    JOIN departments 
