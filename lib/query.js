...
const queries = {
    viewEmployees: `SELECT e.id, e.first_name, e.last_name, IFNULL(emp.last_name,'no manager') as manager, r.salary, r.title, departments.department_name as departmet
        FROM employees e
        LEFT JOIN roles r     
        ON r.id = e.role_id
        JOIN departments 
